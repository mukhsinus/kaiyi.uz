...
export function CtaBand() {
  const { lang } = useLang();
  return (
    <section className="bg-[color:var(--obsidian)] border-b border-[color:var(--border)] py-20 md:py-28">
      <div className="mx-auto max-w-[1400px] px-6 md:px-8 flex flex-col md:flex-row md:items-end justify-between gap-10">
...
export function Footer() {
  const { lang } = useLang();
  return (
    <footer className="bg-[color:var(--graphite)] pt-14 pb-10">
      <div className="mx-auto max-w-[1400px] px-6 md:px-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 pb-10 border-b border-[color:var(--border)]">
          ...
          <nav className="flex flex-wrap gap-x-7 gap-y-2 text-[11px] font-semibold tracking-[0.18em] uppercase text-[color:var(--steel)]">
            <a href="#" className="hover:text-[color:var(--pearl)]">{lang === "uz" ? "Model X7" : "Модель X7"}</a>
            ...
            <a href="#" className="hover:text-[color:var(--pearl)]">{lang === "uz" ? "Aloqa" : "Контакты"}</a>
